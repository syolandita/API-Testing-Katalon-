...
response = WS.sendRequest(findTestObject('Object Repository/Author/Get Author'));

// Verifikasi status code
WS.verifyResponseStatusCode(response, 200)

// Verifikasi jumlah data tidak lebih dari 598
jmlElement = WS.getElementsCount(response, "")

assert jmlElement <= 598
...
for (int i = 0; i < jmlElement; i++ ) {
	...
	idBook = WS.getElementPropertyValue(response, "["+i+"].idBook");
	firstName = WS.getElementPropertyValue(response, "["+i+"].firstName");
	lastName = WS.getElementPropertyValue(response, "["+i+"].lastName");
